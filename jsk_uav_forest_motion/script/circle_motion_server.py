...
class CircleMotion:
    ...
    def init(self):
        ...
        if self.use_dji_ == True:
            ...
            rospy.init_node('circle_motion_server', anonymous=True)
    ...
    def controlCallback(self, event):
        ...
        if self.state_machine_ == self.APPROACHING_TO_TREE_STATE_:
            ...
            self.setGoal(self.LOCAL_FRAME_, np.array([self.tree_xy_pos_[0] - self.circle_radius_*math.cos(tree_direction), self.tree_xy_pos_[1] - self.circle_radius_*math.sin(tree_direction)]),
        ...
        #use local frame
            # vel_msg = self.goPos(self.LOCAL_FRAME_, np.array([self.tree_xy_pos_[0] - self.circle_radius_, self.tree_xy_pos_[1]]), self.takeoff_height_, self.uav_yaw_ + math.atan2(self.tree_xy_pos_[1], self.tree_xy_pos_[0]))
            tree_direction = math.atan2(self.tree_xy_pos_[1], self.tree_xy_pos_[0])
            self.setGoal(self.LOCAL_FRAME_, np.array([self.tree_xy_pos_[0] - self.circle_radius_*math.cos(tree_direction), self.tree_xy_pos_[1] - self.circle_radius_*math.sin(tree_direction)]), 
                                                    self.takeoff_height_,
                                                    self.uav_yaw_ + math.atan2(self.tree_xy_pos_[1], self.tree_xy_pos_[0]))
        ...
        #use local frame
#            self.goPos(self.LOCAL_FRAME_, self.tree_xy_pos_ - self.initial_target_tree_xy_pos_, self.takeoff_height_, self.initial_yaw_)
            self.setGoal(self.GLOBAL_FRAME_, self.initial_xy_pos_, self.takeoff_height_, self.initial_yaw_)
    #end navigation
    ...
    #publication
        ...
        if self.state_machine_ == self.TAKEOFF_STATE_ or self.state_machine_ == self.TREE_DETECTION_START_STATE_ or  self.state_machine_ == self.START_CIRCLE_MOTION_STATE_:
